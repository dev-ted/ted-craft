...
- In public docs and marketplace copy, prefer "subagent" framing over internal "bundle" naming for skill+agent packages.
...
- Production web app is at `https://tedcraft.vercel.app`.
...
- React UI Cursor rules publish as category `rules` (individual rule artifacts plus index skill `react-ui-rules`), not as a frontend category.

## Cursor Cloud specific instructions

- Toolchain: Node 24 and Bun are pre-provisioned in the VM snapshot. Because the base image ships a `/exec-daemon/node` (v22) that would otherwise shadow it, Node 24 (`node`/`npm`/`npx`) and `bun`/`bunx` are symlinked into `/usr/local/cargo/bin` so they resolve ahead of the base image's paths in any shell.
- Commands are the standard ones in `README.md` / root `package.json`: `bun run dev` (web on `http://localhost:3000`), `bun run generate`, `bun run validate`, `bun run types:check`, `bun run lint`, and CLI commands (`bun run cli -- list / search / add`).
- Non-obvious build ordering: the web app statically imports `apps/web/src/data/registry.json`, so the `generate` step (from `@ted-craft/validate`) must run before the web app can build/serve. `bun run dev` and `bun run build` handle this internally.
